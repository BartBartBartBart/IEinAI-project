...
class SRTree(Env):
    def __init__(self, X: torch.Tensor, y: torch.Tensor, action_space: Action,
                 ...
                 update_interval=1000, inner_loop_config=None, verbose=True):
    ...
    def set_up_reward_manager(self):
        ...
        if self.loss == 'nrmse':
            self.reward_manager = NRMSEReward(self, verbose=self.verbose)
        elif self.loss == 'dynamic':
            self.reward_manager = DynamicTSSReward(self, verbose=self.verbose)
        elif self.loss == 'struct':
            self.reward_manager = StructureReward(self, verbose=self.verbose)
        else:
            self.reward_manager = TSSReward(self, verbose=self.verbose)
    ...
    def calc_loss(self, encodings: torch.Tensor):
        ...
        for i in range(n):
            ...
            if cache_loss is None:
                ...
                # quit()
                expressions.append(expression) #expressions is een list en expression is een ExpressionTree
            else:
                loss[i] = cache_loss
