...
class RewardManager(object):
    ...
    def _update_rewards(self, rewards: torch.Tensor, loss: torch.Tensor, encodings: torch.Tensor):
        if len(rewards) > 1 and torch.max(rewards) > self.best_reward:
            ...
            best_encoding = encodings[best_action] #toegevoegd
            if self.verbose:
                print('action_fns: ', self.env.action_fns)#toegevoegd
                print('action_arities: ', self.env.action_arities)#toegevoegd
                print('action_names :',self.env.action_names)#toegevoegd
            ...
            if self.verbose:
                ...
                print(f"best encoding: {str(best_encoding)}") #toegevoegd
...
class DynamicTSSReward(RewardManager):
    ...
    def calc_rewards(self, loss: torch.Tensor, encodings: torch.Tensor, is_eval: bool = False):
        ...
        # check for NaN or Inf values in rewards
        if not torch.isfinite(rewards).all():
            print("Warning: Non-finite rewards detected. Replacing with small value.")
